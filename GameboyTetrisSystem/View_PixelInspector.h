...
class View_PixelInspector : public EView
{
	...
	ivec2 m_PBI_XLimits{ 0, 160 };
	ivec2 m_PBI_YLimits{ 0, 144 };

	ivec2 m_PixelPosition{ 80, 72 };
	int m_NrSidePixels{ 2 };

	void DrawPixelBufferInspector()
	{
		...
		SDL_UpdateTexture(m_pPixelBufferTexture, nullptr, static_cast<void*>(pixelBuffer), 160 * sizeof(uint16_t));

		ImGui::Image(m_pPixelBufferTexture, { 160, 144 });
		ImGui::SameLine();
	...
	void DrawPixelInspector()
	{
		...
		for (int pw{}; pw <= nrPixels; pw++)
		{
			for (int ph{}; ph < nrPixels; ph++)
			{
				...
				if (pixelPos.x > 0 && pixelPos.x < 160 && pixelPos.y > 0 && pixelPos.y < 144)
				{
